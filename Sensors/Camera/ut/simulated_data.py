...
from TrackingUt import random_dict_delete, cart_to_polar, simulated_cones_to_numpy
...
def first_frame(track_func, car_t=0.0, car_angle=0.35 * np.pi, fov=0.3 * np.pi, samples=8, cone_distance=4, eps=0.02,
                plot=False) -> Tuple[np.ndarray, Car, List[SimulatedCone]]:
    ...
    sim_cones = car.filter_out_of_fov(cones)
    ...
    polar_points = np.concatenate((color_col.reshape(len(sim_cones), 1), polar_points), axis=1)

    # visualize cones
    if plot:
        fig, axs = plt.subplots(nrows=1, ncols=2, figsize=(8, 4), sharex=False, sharey=False)
        visualize_ground_truth(axs[0], plot_cones, car=car, show_plot=False, subplot_title="Cart ground truth", first_column_color=False, plot_view_angle=True)
        visualize_detection(axs[1], polar_points, show_plot=False, car=car, plot_car=False, first_col_colors=True)
        plt.show()

    return polar_points, car, sim_cones
